...
export const TicketSummaryBar = ({ data }: TicketSummaryBarProps) => {
    ...
    return (
        <div className="flex w-full overflow-hidden rounded-lg border border-mono-light-grey bg-mono-white shadow-sm">
            {data.map((item, _) => (
                <div key={item.status_id} className="flex flex-1 flex-col">
                    ...
                    >
                        <Text variant="body-sm" weight="bold" className="text-white">
                            {item.status_name}
                        </Text>
                    </div>
